      ...
      <section className="grid gap-6 lg:grid-cols-3 pb-10">
        <div className="lg:col-span-2 rounded-3xl p-8 border border-border bg-card shadow-soft relative overflow-hidden group">
          <div className="flex items-center justify-between mb-6 relative z-10">
            <div>
              <h2 className="font-display text-2xl font-bold text-foreground tracking-tight">Recent activity</h2>
          ...
          <ul className="space-y-1 relative z-10">
            {recentActivity.map((a, i) => (
              <li key={i} className="flex items-center gap-4 py-4 px-2 rounded-xl hover:bg-muted transition-colors border-b border-border/50 last:border-0 group/item">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-muted border border-border group-hover/item:bg-primary/10 group-hover/item:border-primary/20 transition-all duration-300">
                  <a.icon className="h-4 w-4 text-primary" />
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-foreground group-hover/item:text-primary transition-colors">{a.text}</p>
                  <p className="text-xs text-muted-foreground mt-1">{a.time}</p>
                </div>
                <ArrowRight className="h-4 w-4 text-muted-foreground opacity-0 group-hover/item:opacity-100 transition-all -translate-x-2 group-hover/item:translate-x-0" />
              </li>
            ))}
          </ul>
        </div>
